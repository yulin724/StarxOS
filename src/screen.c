...
void putchar_at_screen_row_col(int index, u16int row, u16int col, u8int color, u8int c)
{
    struct screen_struct *p;

    p = &screens[index];
    *(p->mem + row * COL_LEN * 2 + col*2 ) = c;
    *(p->mem + row * COL_LEN * 2 + col*2 +1 ) = color;
}
...
void putstring_bochsconsole(u8int *string)
{
    u8int *p;
